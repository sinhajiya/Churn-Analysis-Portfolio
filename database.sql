...
SELECT 
    Churn_Category, 
    COUNT(Churn_Category) AS total_count,
    COUNT(Churn_Category) / (SELECT COUNT(*) FROM customer_data) * 100 AS Percentage
FROM 
    customer_data
GROUP BY 
     Churn_Category
...
-- Remove null and insert the new data into a new table
CREATE TABLE churn_analysis_db.prod_Churn AS
SELECT 
    Customer_ID,
    Gender,
    Age,
